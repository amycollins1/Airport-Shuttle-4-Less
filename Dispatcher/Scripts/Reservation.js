...
var IsHalt = false, HourlySettingID = 0, HaltingHours = 0, HaltingDiscount = 0, TotalDistanceArrP2P = []; var MaxDiscount = 30;
var ExtraBags = 0, ExtraBagCharge = 0, ExtraAdult = 0, ExtraAdultCharge = 0, ExtraChild = 0, ExtraChildCharge = 0;
var LateNightFixCharges = 10, Adult = 0, Child = 0, CardProcessingPercent = 3.0, CardProcessingAmount = 0;
...
function LoadReservationData() {
    ...
    $.ajax({
        type: "POST",
        url: "/Admin/Handler/AdminHandler.asmx/GetReservation",
        ...
        success: function (response) {
            ...
            if (obj.retCode == 1) {
                ...
                var Vehicle = $.grep(VehicleList, function (p) { return p.Sid == ReservationDetail.VehicleId })[0];
                if (ReservationDetail.Service == 'Frederick-From Airport' || ReservationDetail.Service == 'Frederick-To Airport') {
                    ...
                    if (parseInt(Vehicle.MinCapacity) >= ReservationDetail.Adults) {
                        RemainingCapacity = parseInt(Vehicle.MinCapacity) - ReservationDetail.Adults;
                        ExtraChild = ReservationDetail.Childs - RemainingCapacity
                    }
                    else {
                        ExtraAdult = ReservationDetail.Adults - parseInt(Vehicle.MinCapacity)
                        ExtraChild = ReservationDetail.Childs
                    }
                    //ExtraAdult = parseInt(Vehicle.MinCapacity) - parseInt(ReservationDetail.Adults);
                ...
                else
                {
                    if (ReservationDetail.Service == 'From Airport') {
                        GetA4SLAirports(ReservationDetail.Source); 
                    }
                    else {
                        GetA4SLAirports(ReservationDetail.Destination); 
                    }
                    $("#Select_Service").val(ReservationDetail.Service);
                    GetAllVehicle()
                }
                ...
                if (ReservationDetail.Service == 'Hourly')
                {
                    $('#SelHours').val(ReservationDetail.Hours);
                }
                ...
                if (ReservationDetail.Service == 'From Airport'){
                    ...
                    $('#PickupLocationHourly').val(ReservationDetail.Source);
                    $('#DropLocationHourly').val(ReservationDetail.Destination);
                }
...
function AddReservation() {
    ...
    document.getElementById("BtnAddResv").disabled = true;
    $("#BtnAddResv").text("Please Wait");
    if (DriverId != 0)
        Status = "Confirmed";
    DataArr = {
        Sid: Sid,
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (obj.retCode == 1) {
                ...
                //document.getElementById("BtnAddResv").disabled = false;
                //$("#BtnAddResv").text("ADD"); 
                //window.location.href = "/Dis/ReservationReport.aspx";
            }
...
function GetHaltType() {
    ...
    if (HourlyType == 2) {
        ...
        if (HaltDeductionList.length == 0) {
            $("#HaltType").val(1)
            document.getElementById('txt_HaltHours').disabled = true
            alert("Halt Setting Not Available for this Vehicle!")
        }
    }
    else {
        document.getElementById('txt_HaltHours').disabled = true
            ...
            for (var h = 0; h < HaltDeductionList.length; h++) {
                if (HaltingHours >= HaltDeductionList[h].MinHours && HaltingHours <= HaltDeductionList[h].MaxHours) {
                    ...
                    if (parseFloat(HaltDeductionList[h].PerHourDiscount) > MaxDiscount) {
                        HaltDiscount = parseFloat(MaxDiscount).toFixed(2);
                    }
                }
            } 
        }
...
var IsCCChange = false;
function ChangeCCFees()
{
    ResetCardProcessing();
    CardProcessingAmount = $("#CardProcessingFee").val();
    Total = (parseFloat(Total) + parseFloat(CardProcessingAmount)).toFixed(2);
    IsCCChange = true;
    $("#Total").val(Total)
}
