...
var ExtraBags = 0, ExtraBagCharge = 0, RetExtraBagCharge = 0.0, LateNightFixCharges = 15;
...
var IsChildSeat = false, ChildSeatCharge = 0, IsPetinCage = false, PetinCageCharge = 0,ChildSeatType = "";
        ...
        success: function (response) {
            ...
            if (obj.retCode == 1) {
                ...
                if (List.length > 0) {
                    ...
                    var ExtraBags = parseInt(MySearch.Bags) - parseInt(MySearch.MinCapacity)
...
function LoadData() {
    ...
    if (MySearch.Tab == 1 || MySearch.Tab == 2) {
        ...
        $("#dolist").empty();
        var div = '<ul><li style="margin-bottom:0px;"> AS4L imposes a service fee for late night service (10:00PM TO 5:00AM).<br /></li>';
        div += ' </ul>';
        $("#dolist").append(div);
    ...
    if (MySearch.Tab == 5)
    {
        $("#dolist").empty();
        var div = '<ul><li style="margin-bottom:0px;"> AS4L imposes a service fee for late night service (10:00PM TO 5:00AM).<br /></li>';
        div += ' <li style="margin-bottom:0px;">Each passenger is allowed one suitcase and one personal item. If you have additional suitcases, please select the "Extra Bags" option on Previous Page!</li></ul>';
        $("#dolist").append(div);
...
function ChildSeatChange() {
    ...
    if (ChildSeatType == "-")
    {
        document.getElementById("ChkChildSeat").checked = false;
        //$("#ChkChildSeat").is(":checked") = false
        ChildSeatType = "";
    }
    else
    {
        document.getElementById("ChkChildSeat").checked = true;
    ...
    if (IsChildSeat) {
        if (MySearch.ChkRetReservation == true) {
            $("#ChildSeat").text("$ 40")            
            ChildSeatCharge = parseFloat(childseatrate).toFixed(2);
...
function ResetCalculation()
{
    ...
    if (MySearch.ChkRetReservation) {
        Total = ((parseFloat(SubTotal) * 2) + parseFloat(nSnowCharge) + parseFloat(nGratuityCharge) + parseFloat(nExtraBagCharge) + parseFloat(nExtraAdultCharge) + parseFloat(nExtraChildCharge) + parseFloat(nChildSeatCharge) + parseFloat(nPetInCageCharge)) - parseFloat(nOfferCharge);
    }
    else {
        Total = (parseFloat(SubTotal) + parseFloat(nSnowCharge) + parseFloat(nGratuityCharge) + parseFloat(nExtraBagCharge) + parseFloat(nExtraAdultCharge) + parseFloat(nExtraChildCharge) + parseFloat(nChildSeatCharge) + parseFloat(nPetInCageCharge)) - parseFloat(nOfferCharge);
    }     
}
