...
google.maps.event.addDomListener(window, 'load', function () {
    ...
    google.maps.event.addListener(PlaceLocation, 'place_changed', function () {
        ...
        var AirportLatLong = ($("#SelAirport option:selected").val()).split(',').map(s => s.trim());
        //Airport = $("#SelAirport option:selected").text();
        Airport = AirportLatLong.slice(3).join(',').trim();
        if (Airport == '') {
    ...
    new google.maps.places.SearchBox(document.getElementById('FredrickAddress'));
    var FredrickLocation = new google.maps.places.Autocomplete(document.getElementById('FredrickAddress'));

    google.maps.event.addListener(FredrickLocation, 'place_changed', function () {
        var place = FredrickLocation.getPlace();
        var address = place.formatted_address;
        latitude = place.geometry.location.lat();
        longitude = place.geometry.location.lng(); 
    });
    ...
    //*********Contact Address**********************//
    new google.maps.places.SearchBox(document.getElementById('pick_up_location'));
    var pick_up_location = new google.maps.places.Autocomplete(document.getElementById('pick_up_location'));

    new google.maps.places.SearchBox(document.getElementById('destination'));
    var drop_location = new google.maps.places.Autocomplete(document.getElementById('destination'));

    google.maps.event.addListener(pick_up_location, 'place_changed', function () {
        var place = pick_up_location.getPlace();
        var address = place.formatted_address;
        latitude = place.geometry.location.lat();
        longitude = place.geometry.location.lng();
    });
