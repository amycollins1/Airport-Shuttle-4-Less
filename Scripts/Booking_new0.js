...
let IsLateNight = false,
    IsLateNightRet = false,
    OutboundLateNightCharge = 0,
    ReturnLateNightCharge = 0;
let IsMeetAndGreet = false, IsMeetGreet = false, IsMeetGreetRet = false, Total = 0.0, IsSanitization = false;
...
function LoadData() {
    ...
    if (MySearch.Tab === 1 || MySearch.Tab === 2) {
        ...
        OutboundLateNightCharge = IsLateNight ? LateNightFixCharges : 0;
        $lateNightCharge.text("$ " + OutboundLateNightCharge);
        Total = (parseFloat(MySearch.SubTotal) + parseFloat(OutboundLateNightCharge)).toFixed(2);
        ...
        if (MySearch.ChkRetReservation) {
            ...
            ReturnLateNightCharge =
                IsLateNightRet ? LateNightFixCharges : 0;

            let totalLateNightCharge =
                parseFloat(OutboundLateNightCharge || 0)
                + parseFloat(ReturnLateNightCharge || 0);

            $lateNightCharge.text(
                "$ " + totalLateNightCharge.toFixed(2)
            );

            if (IsLateNightRet) {
                Total = (
                    parseFloat(Total)
                    + parseFloat(ReturnLateNightCharge)
                ).toFixed(2);
            }
            if (MySearch.Tab === 1) {
    ...
    if (MySearch.Tab === 5) {
        ...
        OutboundLateNightCharge =
            IsLateNight ? LateNightFixCharges : 0;

        $lateNightCharge.text(
            "$ " + parseFloat(OutboundLateNightCharge).toFixed(2)
        );

        Total = (
            parseFloat(MySearch.SubTotal)
            + parseFloat(OutboundLateNightCharge)
        ).toFixed(2);
        if (MySearch.ChkRetReservation) {
            ShowHideDiv('DivRetSubTotal');
            IsLateNightRet = LateNightClac(MySearch.RetTime);
            ReturnLateNightCharge = IsLateNightRet ? LateNightFixCharges : 0;

            $lateNightCharge.text("$ " + (parseFloat(OutboundLateNightCharge)
                + parseFloat(ReturnLateNightCharge)).toFixed(2));
...
function ReservationData() {
    ...
    if (MySearch.ChkRetReservation) {
        ...
        let outboundTotal = calculateLegTotal({
            ...
            MeetAndGreet: IsMeetAndGreet ? 30 : 0,
            Offer: OfferAmount,
    ...
    DataArr = {
        ...
        TotalFare: parseFloat(Total || 0).toFixed(2),
...
function RetReservationData() {
    ...
    DataArr = {
        ...
        VehicleId: MySearch.VehicleId,
        VehicleRate: MySearch.VehicleRate,
        BaseCharge: parseFloat(MySearch.BaseCharge || 0),
        Distance: MySearch.TotalDistance,
        Fare: MySearch.RetSubTotal || MySearch.SubTotal,
        ...
        Childs: ExtraChild,
        ChildCharge: ExtraChildCharge,
        TotalFare: parseFloat(TotalRet || 0).toFixed(2),
        OfferDetail: OfferDetails,
        IsPaid: true,
...
function calculateLegTotal(opts) {
    ...
    // Card processing fee
    let cardFee = ((total / 100) * parseFloat(opts.CardProcessingPercent || 0));
    total += cardFee;
    return parseFloat(total).toFixed(2);
}

function CalculateFinalPaymentAmount() {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (obj.retCode == 1) {
                ...
                $('#PayAmount').text(
                    "Total Amount: $ " + CalculateFinalPaymentAmount()
                );
                if (Fname == '') {
...
function LocalData() {
    ...
    MySearch = {
        ...
        TimeTaken: "54 mins",
        SubTotal: "109.94",
        Source: "9618 Melrose Square Way, Gaithersburg, MD, USA",
        Destination: "Baltimore Washington International Airport (BWI)",
        SourceLat: 39.1819,
        SourceLongt: -77.1985,
        DestinationLat: 39.1754,
        DestinationLongt: -76.6684,
        TotalDistance: 58.7,
        VehicleId: "1",
        VehicleRate: 1.95,
