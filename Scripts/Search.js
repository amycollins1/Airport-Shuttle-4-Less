function OnLoadSearchHtml() {
    ...
    if (MyList != "" && MyList != null) {
        ...
        if (isLocal) {
            ...
            // Optionally scroll to selectRide
            if ($('.selectRide').length) {
...
var isLocalTest = true; // Set to false for production
...
function Redirect(Id) {
    ...
    if (MySearch.Tab == 5) {
        ...
        var Adult = $("#txtExtraAdult" + Vehicle[0].VehicleID).val();
        var Child = $("#txtExtraChild" + Vehicle[0].VehicleID).val();
        var Bag = $("#txtExtraBag" + Vehicle[0].VehicleID).val();
...
function LoadVehDesign(List) {
    ...
    for (var i = 0; i < List.length; i++) {
        ...
        tr += '<div class="col-lg-4 col-md-6 col-sm-12 col-12">'
        tr += '<div class="rideCard">'
        tr += '<div class="rideImg">'
        tr += '<img src="/images/VehicleImages/' + List[i].Sid + '.jpg" alt=' + List[i].Model + ' class="w-100">'
        tr += '</div>'
        tr += '<div class="rideDetail">'
        tr += '<div class="rideDetailHead d-flex align-items-center justify-content-between mb-3">'
        tr += '<div>'
        tr += '<h4 class="level4 mb-0 primaryColor">' + List[i].Model + '</h4>'
        tr += '<h5 class="level5 mb-0 secondaryColor">Price: $' + Price + '</h5>'
        tr += '</div>'
        tr += '<button class="primaryButton bookNowBtn" onclick="Redirect(\'' + List[i].Sid + '\')">Book Now</button>'
        tr += '</div>'
        tr += '<ul class="list-inline mb-0 d-flex flex-wrap justify-content-between">'
        tr += '<li class="w-50"><i class="fa fa-user me-2"></i>Maximum Capacity: ' + List[i].MaxCapacity + '</li>'
        tr += '<li class="w-50"><i class="fa fa-suitcase me-2"></i>Max Baggage: ' + List[i].MaxBaggage + '</li>'
        tr += '<li class="w-50"><i class="fa fa-wifi me-2"></i>On Board WiFi</li>'
        tr += '<li class="w-50"><i class="fa fa-mobile me-2"></i>Phone Chargers</li>'
        tr += '<li class="w-50"><i class="fa fa-bottle-water me-2"></i>Complimentary Water / Newspaper</li>'
        tr += '<li class="w-50"><i class="fa fa-chair me-2"></i>Heated Seats</li>'
        tr += '</ul>'
        tr += '</div>'
        tr += '</div>'
        tr += '</div>'
    ...
    tr += '<div class="col-lg-12 col-md-12 col-sm-12 col-12">'
    tr += '<button class="secondaryButton mx-auto d-table mt-5">Back</button>'
    tr += '</div>'
...
function LoadFredrickVehicles() {
    $("#VehicleList").empty();
    var tr = '';
    var ServiceType = MySearch.Service;
    var Tab = MySearch.Tab;
    var FromID = parseInt(MySearch.FredrickSourceID);
    var ToID = parseInt(MySearch.FredrickDestinationID);
    var Passengers = MySearch.Passengers;
    var data = { Service: ServiceType, Tab: Tab, From: FromID, To: ToID, Passengers: Passengers }
    $.ajax({
        type: "POST",
            ...
            if (obj.retCode == 1) {
                ...
                if (Tab == "5") {
                    for (var i = 0; i < List.length; i++) {
                        ...
                        tr += '<div class="col-lg-4 col-md-6 col-sm-12 col-12">'
                        tr += '<div class="rideCard">'
                        tr += ' <div class="rideImg"><img src="images/VehicleImages/' + List[i].VehicleID + '.jpg" class="w-100"></div>'
                        tr += ' <div class="rideDetail">'
                        tr += '<div class="rideDetailHead d-flex align-items-center justify-content-between mb-3">'
                        tr += '<div><h4 class="level4 mb-0 primaryColor">' + List[i].VehicleModel + '</h4><h5 class="level5 mb-0 secondaryColor">Price: $' + List[i].BaseRate + '</h5></div>'
                        tr += '<button class="primaryButton bookNowBtn" onclick="Redirect(\'' + List[i].RateID + '\')">Book Now</button>'
                        tr += '</div>'
                        tr += '<ul class="list-inline mb-0 d-flex flex-wrap justify-content-between">'
                        tr += ' <li class="w-100"><i class="fa fa-user me-2"></i>Maximum Passengers Capacity: ' + List[i].MaxCapacity + '</li>'
                        tr += ' <li class="w-100"><i class="fa fa-user me-2"></i>Passengers Allowed(Base Price): ' + List[i].MinCapacity + '</li>'
                        tr += '<li class="w-100"><i class="fa fa-suitcase me-2"></i>Max Baggage Capacity: ' + List[i].MaxBaggage + '</li>'
                        tr += '<li class="w-100"><i class="fa fa-suitcase me-2"></i>Baggage Allowed(Base Price): ' + List[i].MinBaggage + '</li>'
                        tr += '<hr/>'
                        tr += '<p class="mb-3 primaryText greyText">Note: Please Add these if you have more than allowed Passengers / Bags</p>'
                        

                        tr += '<div class="row">'
                        tr += '<div class="col-lg-12 col-md-12 col-sm-12 col-12">'
                        tr += '<div class="form-group form-counter">'
                        tr += '<label class="text-decoration-underline primaryColor">Extra Adult (' + ListADCH[0].AdultRate + ' $/Pessenger)</label>'
                        tr += '<div class="counter">'
                        tr += '<span class="down" onClick="minusAdult(' + List[i].VehicleID + ')">-</span>'
                        tr += '<input type="number" value="0" class="form-control text-center" id="txtExtraAdult' + List[i].VehicleID + '" min="0" max=' + List[i].MaxCapacity + '>'
                        tr += '<span class="up" onClick="plusAdult(' + List[i].VehicleID + ',' + ExtraCapacity + ')">+</span>'
                        tr += '</div>'
                        tr += '</div>'
                        tr += '</div>'
                        tr += '<div class="col-lg-12 col-md-12 col-sm-12 col-12">'
                        tr += '<div class="form-group form-counter">'
                        tr += '<label class="text-decoration-underline primaryColor">Extra Child (' + ListADCH[0].ChildRate + ' $/Pessenger)</label>'
                        tr += '<div class="counter">'
                        tr += '<span class="down" onClick="minusChild(' + List[i].VehicleID + ')">-</span>'
                        tr += '<input type="number" value="0" class="form-control text-center" id="txtExtraChild' + List[i].VehicleID + '" step="1" min="0" max=' + List[i].MaxCapacity + '>'
                        tr += '<span class="up" onClick="plusChild(' + List[i].VehicleID + ',' + ExtraCapacity + ')">+</span>'
                        tr += '</div>'
                        tr += '</div>'
                        tr += '</div>'
                        tr += '<div class="col-lg-12 col-md-12 col-sm-12 col-12">'
                        tr += '<div class="form-group form-counter">'
                        tr += '<label class="text-decoration-underline primaryColor">Extra Bag (' + ListADCH[0].BaggageRate + ' $/Bag)</label>'
                        tr += '<div class="counter">'
                        tr += '<span class="down" onClick="minusBag(' + List[i].VehicleID + ')">-</span>'
                        tr += '<input type="number" value="0" class="form-control text-center" id="txtExtraBag' + List[i].VehicleID + '" step="1" min="0" max=' + List[i].MaxBaggage + '>'
                        tr += '<span class="up" onClick="plusBag(' + List[i].VehicleID + ',' + ExtraBaggage + ')">+</span>'
                        tr += '</div>'
...
// UI developer increament decrement code
...
function MapDesigning() {
    ...
    window.onload = function () {
        ...
        var GoogleService = new google.maps.DirectionsService();
...
function localData() {
    MySearch = {
        ...
        DestinationLatRet: 39.1774,
        DestinationLongt: -76.6684,
        DestinationLongtRet: -76.6684,
        FlightNo: "DL123",
        HaltingDiscount: 0,
        HaltingHours: 0,
        HourlySettingID: 0,
        ...
        SourceLatRet: 38.9071923,
        SourceLongt: -77.0368707,
        SourceLongtRet: -77.0368707,
        Stops: "Midway Stop",
        SubTotal: 100,
        Tab: 1,
        Time: "08:00:AM",
        TimeTaken: "49 mins",
        TimeTakenRet: "50 mins",
        TotalDistance: 35.67,
        TotalDistanceRet: 35.67,
        VehicleId: 1,
        VehicleRate: 2.79,
        destinationP2PRet: "Baltimore, MD, USA",
        sourceP2PRet: "Washington D.C., DC, USA",
        // P2P/Hourly fields
        LocationP2PArr: ["Washington D.C., DC, USA", "Baltimore, MD, USA", "Philadelphia, PA, USA"],
        LatLongP2PArr: ["38.9071923^-77.0368707", "39.2903848^-76.6121893", "39.9525839^-75.1652215", "", ""],
        DistanceP2PArr: [35.67, 50.12, 0, 0],
        // Frederick fields
        FredrickSourceID: 1,
        FredrickDestinationID: 2,
        FredrickAddress: "123 Main St, Frederick, MD",
        Bags: 2,
        MinBaggage: 1,
        MaxBaggage: 4,
        MinCapacity: 2,
